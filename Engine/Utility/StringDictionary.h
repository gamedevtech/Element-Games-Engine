...
namespace EG{
    namespace Utility{
        ...
        /* Hash Table Entry */
        // NOTE: This appears to have a memory leak where it's not getting cleared properly!
        ...
        void StringDictionary<HashTableValueType>::Clear(void){
            if (_table != NULL && initialized){
                for (unsigned int i = 0; i < _map_size; i++){
                    if (_table[i] != NULL){
                        StringDictionaryEntry<HashTableValueType> *entry = _table[i];
                        while (entry->GetNext()){
                            StringDictionaryEntry<HashTableValueType> *delete_me = entry;
                            delete delete_me;
                            entry = entry->GetNext();
                        }
                        delete _table[i];
