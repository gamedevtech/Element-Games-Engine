...
// 						}
		...
		glm::vec4 *GenerateSphereNormalHeightMap(unsigned int width, unsigned int height, float *heights, std::string file_path){
			...
			glm::vec4 *normals = new glm::vec4[normal_count];
			...
			for (unsigned int z = 0; z < height; ++z){
				for(unsigned int x = 0; x < width; ++x){
					unsigned int x_left =   (x == 0)            ? (width - 1)  : (x - 1);
					unsigned int x_right =  (x == (width - 1))  ? (0)          : (x + 1);
					...
					//glm::vec3 n = v1.Cross(v2);
					n = glm::normalize(n);
					//n.Normalize();
					n *= normal_map_scalar;
					normals[normal_index] = glm::vec4(n.x, n.y, n.z, heights[(z * width) + x]);
					normal_index += 1;

					if (image_dump){
						unsigned char px = (unsigned char)((n.x * 128.0f) + 128.0f);
						unsigned char py = (unsigned char)((n.y * 128.0f) + 128.0f);
						unsigned char pz = (unsigned char)((n.z * 128.0f) + 128.0f);
						unsigned char pw = 255;//(unsigned char)((heights[(z * width) + x] * 128.0f) + 128.0f);
						pixels[index] = px;
						pixels[index + 1] = py;
						pixels[index + 2] = pz;
						pixels[index + 3] = pw;
...
// 					std::cout << i << std::endl;
		...
		glm::vec4 **GenerateCubeSphereNormalHeightMap(unsigned int width, unsigned int height, float **heights, std::string file_path){
			...
			for (unsigned int i = 0; i < 6; i++){
				...
				std::string first_part = file_path.substr(0, file_path.find_first_of('.'));
				file_path_out << first_part << '_' << i << ".png";
				out[i] = EG::Math::GenerateSphereNormalHeightMap(width, height, heights[i], file_path_out.str());
			}
